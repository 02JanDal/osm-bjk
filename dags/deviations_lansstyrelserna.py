...
for view_name, dataset_name in [("vindbrukskollen_turbines", "vindbrukskollen")]:
    with DAG(
        f"deviations-{view_name}",
        description=f"Updates deviations based on v_deviations_{view_name}",
        schedule=[Dataset(f"psql://upstream/lansstyrelsern/{dataset_name}")],
        ...
        tags=["provider:Länsstyrelserna", "type:Deviations"],
    ):
