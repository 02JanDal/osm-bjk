...
""")
    ...
    print("  Building ways...")
    cursor.execute("""
INSERT INTO osm.way_geom (id, timestamp, geom) 
	SELECT way.id, GREATEST((way.meta).timestamp, (
...
INSERT INTO osm.area (id, geom, way_id, timestamp)
	...
	FROM osm.dirty
	INNER JOIN osm.way_geom ON dirty.way_id = way_geom.id
	WHERE ST_IsClosed(way_geom.geom) AND ST_NPoints(way_geom.geom) > 3
...
INSERT INTO osm.area (id, relation_id, geom, timestamp)
	SELECT relation.id + 3600000000, relation.id, ST_BuildArea(ST_Collect(way_geom.geom)), MAX(way_geom.timestamp)
	FROM osm.dirty
