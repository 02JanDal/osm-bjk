...
TRUNCATE osm.{self._prefix}_way_node;
    ...
    def way(self, w: Way):
        self._buffer.append((w.id, process_tags(w.tags), w.timestamp, w.uid, w.version))
...
TRUNCATE osm.{self._prefix}_relation_relation;
    ...
    def relation(self, r: Relation):
        self._buffer.append((r.id, process_tags(r.tags), r.timestamp, r.uid, r.version))
...
TRUNCATE osm.{self._prefix}_changeset;
    ...
    def changeset(self, c: Changeset):
        self._buffer.append((c.id, process_tags(c.tags), c.created_at, c.open, c.uid))
        self._count += 1
        if len(self._buffer) > 100000:
            self.write_buffer()
