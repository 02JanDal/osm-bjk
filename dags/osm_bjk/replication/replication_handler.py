...
class ReplicationHandler(SimpleHandler):
    ...
    def way(self, w: Way):
        ...
        if w.deleted:
            ...
            self._cursor.execute("SELECT id FROM osm.node WHERE id = ANY (%s)", ([n.ref for n in w.nodes],))
            ...
            if missing_nodes:
                try:
                    ...
                    raise RuntimeError("Could not load missing nodes: " + ", ".join([str(n) for n in missing_nodes])) from e

            self._way_buffer.append((w.id, process_tags(w.tags), w.timestamp, w.uid, w.version, [n.ref for n in w.nodes if n.ref not in deleted_nodes]))
        self._ways_now += 1
