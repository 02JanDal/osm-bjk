...
with DAG(
    ...
    def process(sequence_n: int, sequence_w: int, sequence_r: int, sequence_c: int, url: str, ti: TaskInstance = None):
        ...
        with pg_cursor(ti) as cur:
            ...
            cur.execute(f"INSERT INTO osm.node (id, geom, tags, meta) (SELECT id, ST_Transform(geom, 3006), tags, ROW(timestamp, uid, version)::osm.osm_meta FROM osm.{prefix}_node)")
    ...
    process_t = process(load_nodes_t["sequence"], load_ways_t["sequence"], load_relations_t["sequence"], load_changesets_t["sequence"], load_nodes_t["url"])
    cleanup_t = cleanup()
