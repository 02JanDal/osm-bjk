CREATE OR REPLACE VIEW upstream.v_deviations AS
 SELECT q.dataset_id::bigint,
   ...
   FROM ( SELECT v_deviation_preschools_scb.dataset_id,
            ...
            v_deviation_preschools_scb.upstream_item_id,
        ...
        UNION ALL
         SELECT v_deviation_trees_gavle.dataset_id,
            ...
            v_deviation_trees_gavle.upstream_item_id,
        ...
        UNION ALL
         SELECT v_deviation_schools_skolverket.dataset_id,
            ...
            v_deviation_schools_skolverket.upstream_item_id,
...
BEGIN
	SELECT * INTO ups FROM upstream.v_deviations WHERE dataset_id = $1.dataset_id AND layer_id = $1.layer_id AND upstream_item_id IS NOT DISTINCT FROM $1.upstream_item_id;-- AND osm_element_id IS NOT DISTINCT FROM $1.osm_element_id AND osm_element_type IS NOT DISTINCT FROM $1.osm_element_type;
	IF ups.dataset_id IS NULL THEN
		IF EXISTS(SELECT 1 FROM upstream.item WHERE dataset_id = $1.dataset_id AND id = $1.upstream_item_id) THEN
...
CREATE OR REPLACE FUNCTION upstream.sync_deviations(view_name text, dataset_id BIGINT) RETURNS record
    LANGUAGE sql
    AS $_$
WITH deviations AS (SELECT * FROM upstream._dynamic_deviations_by_view_name(view_name)),
	 insertions AS (
		 INSERT INTO api.deviation (dataset_id, layer_id, upstream_item_id, suggested_geom, suggested_tags, osm_element_id, osm_element_type, title, description)
		 SELECT deviations.dataset_id, layer_id, upstream_item_id, suggested_geom, suggested_tags, osm_element_id, osm_element_type, title, description FROM deviations
		 ON CONFLICT ON CONSTRAINT uniq DO UPDATE SET suggested_geom = EXCLUDED.suggested_geom, suggested_tags = EXCLUDED.suggested_tags, description = EXCLUDED.description
		 RETURNING *
	 ),
	 deletions AS (
		 DELETE FROM api.deviation
	 	 WHERE deviation.dataset_id = $2 AND (deviation.action IS NULL OR deviation.action = 'deferred')
		   AND NOT EXISTS (SELECT 1 FROM deviations WHERE deviation.dataset_id = deviations.dataset_id AND deviation.layer_id = deviations.layer_id AND deviation.upstream_item_id = deviations.upstream_item_id AND deviation.osm_element_id = deviations.osm_element_id AND deviation.osm_element_type = deviations.osm_element_type AND deviation.title = deviations.title)
		 RETURNING *
	 )
