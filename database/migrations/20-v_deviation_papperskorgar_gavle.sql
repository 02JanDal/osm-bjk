...
CREATE OR REPLACE VIEW upstream.v_match_papperskorgar_gavle AS
	WITH gavle AS (
		...
		WHERE element.tags->>'amenity' = 'waste_basket' AND ST_Within(geom, (SELECT gavle.geom FROM gavle)) AND type = 'n'
	), ups_objs AS (
...
ALTER TABLE upstream.mv_match_papperskorgar_gavle OWNER TO app;

CREATE OR REPLACE VIEW upstream.v_deviation_papperskorgar_gavle AS
	SELECT
		31::bigint AS dataset_id,
		...
		osm_element_type,
		CASE
			WHEN osm_element_id IS NULL THEN 'Papperskorg saknas'::text
			ELSE 'Papperskorg saknar taggar'::text
		END AS title,
		CASE
			WHEN osm_element_id IS NULL THEN 'Enligt Gävle kommun ska det finnas en papperskorg här'::text
			ELSE 'Följande taggar, härledda ur från Gävle kommuns data, saknas på papperskorgen här'::text
		END AS description,
		 '' AS note
	FROM upstream.mv_match_papperskorgar_gavle
	WHERE osm_tags IS NULL OR upstream_tags IS NULL OR tag_diff(osm_tags, upstream_tags) <> '{}'::jsonb;

CREATE OR REPLACE FUNCTION api.tile_match_papperskorgar_gavle(z integer, x integer, y integer)
    RETURNS bytea
    LANGUAGE 'sql'
...
AS $$
	WITH
		...
		mvtgeom AS (
			...
			FROM upstream.mv_match_papperskorgar_gavle items
