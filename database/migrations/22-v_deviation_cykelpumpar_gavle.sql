CREATE OR REPLACE VIEW upstream.v_match_cykelpumpar_gavle AS
	WITH gavle AS (
		SELECT municipality.geom FROM api.municipality WHERE municipality.code = '2180'
	), osm_objs AS (
		SELECT id, type, tags, geom FROM osm.element
		WHERE element.tags->>'amenity' = 'bicycle_repair_station' AND ST_Within(geom, (SELECT gavle.geom FROM gavle)) AND type = 'n'
	), ups_objs AS (
...
DROP MATERIALIZED VIEW IF EXISTS upstream.mv_match_cykelpumpar_gavle CASCADE;
CREATE MATERIALIZED VIEW upstream.mv_match_cykelpumpar_gavle AS SELECT * FROM upstream.v_match_cykelpumpar_gavle;
ALTER TABLE upstream.mv_match_cykelpumpar_gavle OWNER TO app;
...
CREATE OR REPLACE VIEW upstream.v_deviation_cykelpumpar_gavle AS
	SELECT
		...
		osm_element_type,
		CASE
			WHEN osm_element_id IS NULL THEN 'Cykelpump saknas'::text
			ELSE 'Cykelpump saknar taggar'::text
		END AS title,
		CASE
			WHEN osm_element_id IS NULL THEN 'Enligt Gävle kommun ska det finnas en cykelpump här'::text
			ELSE 'Följande taggar, härledda ur från Gävle kommuns data, saknas på cykelpumpen här'::text
		END AS description,
		 '' AS note
	FROM upstream.mv_match_cykelpumpar_gavle
	WHERE osm_tags IS NULL OR upstream_tags IS NULL OR tag_diff(osm_tags, upstream_tags) <> '{}'::jsonb;

CREATE OR REPLACE FUNCTION api.tile_match_cykelpumpar_gavle(z integer, x integer, y integer)
    RETURNS bytea
    LANGUAGE 'sql'
...
AS $$
	WITH
		...
		mvtgeom AS (
			...
			FROM upstream.mv_match_cykelpumpar_gavle items
