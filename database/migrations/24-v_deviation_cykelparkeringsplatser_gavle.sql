CREATE OR REPLACE VIEW upstream.v_match_cykelparkeringsplatser_gavle AS
	WITH gavle AS (
		SELECT municipality.geom FROM api.municipality WHERE municipality.code = '2180'
	), osm_objs AS (
		SELECT id, type, tags, geom FROM osm.element
		WHERE element.tags->>'amenity' = 'bicycle_parking' AND ST_Within(geom, (SELECT gavle.geom FROM gavle)) AND type IN ('n', 'a')
	), ups_objs AS (
...
CREATE OR REPLACE VIEW upstream.v_deviation_cykelparkeringsplatser_gavle AS
	SELECT
		21::bigint AS dataset_id,
		...
		osm_element_type,
		CASE
			WHEN osm_element_id IS NULL THEN 'Cykelparkeringsplats saknas'::text
			ELSE 'Cykelparkeringsplats saknar taggar'::text
		END AS title,
		CASE
			WHEN osm_element_id IS NULL THEN 'Enligt Gävle kommun ska det finnas en cykelparkeringsplats här'::text
			ELSE 'Följande taggar, härledda ur från Gävle kommuns data, saknas på cykelparkeringsplatsen här'::text
		END AS description,
		 '' AS note
	FROM upstream.mv_match_cykelparkeringsplatser_gavle
	WHERE osm_tags IS NULL OR upstream_tags IS NULL OR tag_diff(osm_tags, upstream_tags) <> '{}'::jsonb;

CREATE OR REPLACE FUNCTION api.tile_match_cykelparkeringsplatser_gavle(z integer, x integer, y integer)
    RETURNS bytea
    LANGUAGE 'sql'
...
AS $$
	WITH
		...
		mvtgeom AS (
			...
			FROM upstream.mv_match_cykelparkeringsplatser_gavle items
