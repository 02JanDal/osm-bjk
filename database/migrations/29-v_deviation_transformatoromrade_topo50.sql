...
CREATE OR REPLACE VIEW upstream.v_deviation_transformatoromrade_topo50 AS
	SELECT
		...
		END AS description,
		 '' AS note
	FROM upstream.mv_match_transformatoromrade_topo50
	WHERE osm_tags IS NULL OR upstream_tags IS NULL OR tag_diff(osm_tags, upstream_tags) <> '{}'::jsonb;

CREATE OR REPLACE FUNCTION api.tile_match_transformatoromrade_topo50(z integer, x integer, y integer)
    RETURNS bytea
    LANGUAGE 'sql'
...
AS $$
	WITH
		...
		mvtgeom AS (
			...
			FROM upstream.mv_match_transformatoromrade_topo50 items
